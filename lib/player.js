...
class SPPlayer {
	...
	constructor(mod) {
		...
		// Combat stats
		mod.hook('S_PLAYER_STAT_UPDATE', mod.patchVersion < 86 ? 12 : 13, HOOK_LAST, event => {
			Object.assign(this, {
		...
		// Skill Polishing
		mod.hook('C_RQ_SKILL_POLISHING_LIST', 1, event => {
			skillPolishing = vent.optionEffects.filter(sp => sp.active).map(sp => sp.id)
		})
