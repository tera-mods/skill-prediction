...
class SPPlayer {
	...
	constructor(mod) {
		...
		mod.hook('S_LOGIN', mod.patchVersion < 81 ? 12 : 13, HOOK_LAST, event => {
			this.reset()
		...
		if(mod.patchVersion < 85)
		{
			...
			mod.hook('S_ITEMLIST', 1, event => {
				if(event.container === 14) {
					this.hasWeapon = event.items.some(i => i.slot === 1)

					this.itemPassives = []
					for(let item of event.items)
						for(let passive of item.passivitySets.find(s => s.index === item.passivitySet).passivities)
							if(passive) this.itemPassives.push(passive)
				}
			})
	}
