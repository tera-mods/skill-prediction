...
const { SkillID, Vec3 } = require('tera-data-parser/types'),
	...
	clientSkils = require('../client_data/skills'),
...
module.exports = function SPCore(mod) {
	const skills = require(`../config/skills${mod.patchVersion < 98 ? '_97' : ''}`)
	...
	function startSkill(type, version, code, data) {
		...
		const event = mod.parse(type, version, data = Buffer.from(data)),
	...
	function handleStartSkill(type, event, info, data) {
		...
		// Cannot cast skills while dead
		if(!alive) {
			sendCannotStartSkill(event.skill)
			return false
		}

		// Skill cannot be cast without a weapon
		...
		// Skill cannot be cast while mounted - TODO: Support mounted skill flag
		if(player.mounted) {
			sendCannotStartSkill(event.skill)
		...
		if(clientAction) {
			...
			if(clientAction.skill.type === TYPE_ACTION) {
				...
				reaction: {
					...
					// Should be (templateId*100 + reactionId), but not always the case due to BHS typos
					else if(Math.floor(clientAction.skill.id / 1000000) === 1 && clientAction.skill.id % 100 === 8) {
						if(info.useSkillWhileBulldozer) break reaction
					}
					else {
						if(Math.floor(clientAction.skill.id / 1000000) === 1 && clientAction.skill.id % 100 === 2 && info.dcType === 'stand') break reaction
		...
		for(let i = 0, chain = info.userChain; i < 10; i++, chain = undefined) { // Limit recursion to prevent infinite loop
			...
			if(chain == null && clientAction) {
				...
				if(categoryChains) {
					const category = clientSkils[clientAction.templateId]?.[clientAction.skill.id]?.category
			...
			if(chain != null) {
				...
				if(chainSkill.equals(skill) || !clientSkils[player.templateId]?.[chainSkill.id]) break
		...
		// Skill can only be chained from a successful block
		...
		// Skill cannot be cast without a target
		...
		const currentSkillCategory = clientSkils[player.templateId]?.[skill.id]?.category
		...
		function runPassive(passive) {
			// Make sure passive exists and that every conditionCategory (if non-null) is a category of our current skill
			if(!passive || passive.conditionCategory?.some(id => !currentSkillCategory.includes(id))) return

			// Hotfix for Berserker EP: Thunder Strike - Speed Skating
			// TODO: if(passive.condition === 92 && !abnormalityKinds.has(passive.conditionValue)) return
			if(passive.condition === 92 && passive.conditionValue === 23 &&
				![400100, 400101, 400102, 400103, 400104, 400105].some(id => abnormality.exists(id))
			)
				return
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	mod.hook('S_EACH_SKILL_RESULT', mod.patchVersion < 86 ? 13 : 14, event => {
	...
	mod.hook('C_CAN_LOCKON_TARGET', 3, event => {
		const info = skillInfo(event.skill)
		if(info)
			mod.send('S_CAN_LOCKON_TARGET', 3, Object.assign({ success: !info.partyOnly || party.hasMember(event.target) }, event))
	})
	...
	function skillInfo(skill) {
		...
		const race = player.race,
			...
			info = [ // Ordered by least specific < most specific
				clientSkils?.[player.templateId]?.[skill.id],
				skills?.['*']?.[skill.id],
				skills?.[job]?.['*'],
				skills?.[job]?.['*']?.level?.[level],
				skills?.[job]?.['*']?.race?.[race],
				skills?.[job]?.['*']?.race?.[race]?.level?.[level],
				skills?.[job]?.[group]?.['*'],
				skills?.[job]?.[group]?.['*']?.level?.[level],
				skills?.[job]?.[group]?.['*']?.race?.[race],
				skills?.[job]?.[group]?.['*']?.race?.[race]?.level?.[level],
				skills?.[job]?.[group]?.[sub],
				skills?.[job]?.[group]?.[sub]?.level?.[level],
				skills?.[job]?.[group]?.[sub]?.race?.[race],
				skills?.[job]?.[group]?.[sub]?.race?.[race]?.level?.[level]
			]
