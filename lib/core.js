...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
		...
		function runAbnormal(abnormal) {
			...
			for(let effect of abnormal.effects)
				switch(effect.type) {
					...
					case 203:
					case 317:
						// Noctenium Infusion - TODO: implement
					...
					case 239: if(hasOverlapCategory(abnormal.bySkillCategory)) aStamina += effect.value; break
				}
		}
		...
		if(currentSkillCategory) {
			for(let id of player.crests) runPassive(passivity[id]) // Technically wrong, but so far they match up 1:1
