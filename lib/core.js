...
const { SkillID, Vec3 } = require('tera-data-parser/types'),
	...
	skillsConfig = require('../config/skills_config'),
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	function sendActionStage(opts) {
		...
		if(multiStage)
			animSeq = animSeq && animSeq[opts.stage] || !opts.moving && get(info, 'inPlace', 'animSeq', opts.stage) || get(info, 'animSeq', opts.stage) || []
		else
			animSeq = animSeq || !opts.moving && get(info, 'inPlace', 'animSeq') || info.animSeq || []
	...
	function skillInfo(skill) {
		...
		if (skillsConfig[job] && skillsConfig[job]["enabled"] === false || skillsConfig[job][group] === false) {
